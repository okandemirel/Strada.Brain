...
| Pattern matcher (`findMatchingErrorPatterns`) | Yes | unit + integration | **No** (off by design) | Lexical/keyword matching tested. Error-pattern learning is intentionally off: no production `tool:result` sets `errorDetails`, so tool failures feed no `error_patterns` rows or "recurring error" instincts, and nothing calls this matcher in production. |
