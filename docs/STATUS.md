...
| Pattern matcher (`findMatchingErrorPatterns`) | Yes | unit + integration | **No** (no production caller) | Lexical/keyword matching tested. Error-pattern learning is on, from structured signatures only: a failed `tool:result` carries category, strict diagnostic code, project-relative file and line with a templated message (`src/learning/error-signature.ts`, re-validated by the pipeline), feeding `error_patterns` and "recurring error" instincts. Nothing calls this matcher in production. |
