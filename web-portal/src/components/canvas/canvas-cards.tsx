...
export function CodeBlockCard({ shape }: CardProps) {
  const { t } = useTranslation('canvas')
  const { code = '', language = 'text', title } = shape.props as Record<string, string>
  const displayTitle = title || t('cards.codeBlock.defaultTitle')
  return (
    <div className={`${cardBase} border-sky-400/15 bg-gradient-to-b from-sky-400/[0.06] to-[#0a0e16]/95`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="border-b border-sky-400/10 bg-gradient-to-b from-white/[0.035] to-white/[0.01] px-3.5 py-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            <CardBadge className="border-sky-400/20 bg-sky-400/10 text-sky-300">{t('cards.codeBlock.badge')}</CardBadge>
            <div className="mt-2 text-sm font-semibold tracking-tight text-white">{displayTitle}</div>
          </div>
  ...
  return (
    <div className={`${cardBase} border-amber-400/15 bg-gradient-to-b from-amber-400/[0.04] to-[#0a0e16]/95`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="border-b border-amber-400/10 bg-gradient-to-b from-white/[0.035] to-white/[0.01] px-3.5 py-3">
        <CardBadge className="border-amber-400/20 bg-amber-400/10 text-amber-300">{t('cards.diffBlock.badge')}</CardBadge>
  ...
  return (
    <div className={`${cardBase} border-white/10 bg-gradient-to-b from-white/[0.03] to-[#0a0e16]/95 p-4`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="flex h-full flex-col justify-between">
        <div>
          <CardBadge className="border-sky-400/15 bg-sky-400/[0.08] text-sky-200">{t('cards.fileCard.badge')}</CardBadge>
        ...
        <div className="flex flex-wrap gap-2">
          <CardBadge className="border-white/8 bg-white/[0.04] text-slate-400">{String(language) || t('cards.fileCard.badge')}</CardBadge>
          <CardBadge className="border-white/8 bg-white/[0.04] text-slate-400">{t('cards.fileCard.lines', { count: Number(lineCount) })}</CardBadge>
        </div>
      </div>
    </div>
  )
...
export function DiagramNodeCard({ shape }: CardProps) {
  const { t } = useTranslation('canvas')
  const { label, nodeType = 'default', status = 'idle' } = shape.props as Record<string, string>
  const displayLabel = label || t('cards.diagramNode.defaultLabel')
      ...
      <div className="flex h-full flex-col justify-between">
        <div className="flex items-center justify-between gap-2">
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.12em] ${statusColors[status] ?? statusColors.idle}`}>
            ...
            {t(`cards.diagramNode.status${status.charAt(0).toUpperCase() + status.slice(1)}` as 'cards.diagramNode.statusActive')}
          </span>
          <span className="text-[9px] font-semibold uppercase tracking-[0.16em] text-slate-600">{nodeType}</span>
        </div>
        <div className="text-lg font-semibold tracking-tight text-white">{displayLabel}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className={`${cardBase} border-emerald-400/15 bg-gradient-to-b from-emerald-400/[0.04] to-[#050810]/95`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="border-b border-emerald-400/10 bg-gradient-to-b from-white/[0.035] to-white/[0.01] px-3.5 py-3">
        <CardBadge className="border-emerald-400/20 bg-emerald-400/10 text-emerald-300">{t('cards.terminalBlock.badge')}</CardBadge>
    ...
    <div className={`${cardBase} border-white/10 bg-gradient-to-b from-white/[0.03] to-[#0a0e16]/95`} style={{ width: shape.w, height: shape.h }}>
      ...
      {safeSrc ? (
        ...
        <div className="flex h-full flex-col items-center justify-center gap-2.5">
          <CardBadge className="border-sky-400/15 bg-sky-400/[0.08] text-sky-200">{t('cards.imageBlock.badge')}</CardBadge>
          <span className="text-xs text-slate-500">{alt || t('cards.imageBlock.noImage')}</span>
        </div>
      )}
    </div>
  )
...
export function TaskCardCard({ shape }: CardProps) {
  const { t } = useTranslation('canvas')
  const { title, status = 'todo', priority = 'medium' } = shape.props as Record<string, string>
  const displayTitle = title || t('cards.taskCard.defaultTitle')
  ...
  return (
    <div className={`${cardBase} border-sky-400/15 bg-gradient-to-b from-sky-400/[0.04] to-[#0a0e16]/95`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="p-4">
        <div className="flex flex-wrap gap-2">
          <CardBadge className="border-white/8 bg-white/[0.04] text-slate-300">{t(`cards.taskCard.status${status.charAt(0).toUpperCase() + status.slice(1)}` as 'cards.taskCard.statusTodo')}</CardBadge>
          <CardBadge className={priorityBadge[priority] ?? priorityBadge.medium}>{t(`cards.taskCard.priority${priority.charAt(0).toUpperCase() + priority.slice(1)}` as 'cards.taskCard.priorityLow')}</CardBadge>
        </div>
        <div className="mt-3 text-base font-semibold leading-snug tracking-tight text-white">{displayTitle}</div>
      </div>
    </div>
  )
}
...
export function NoteBlockCard({ shape }: CardProps) {
  ...
  return (
    ...
    >
      <AiBadge source={shape.source} />
      <CardBadge className="border-amber-400/20 bg-amber-400/10 text-amber-300" >{t('cards.noteBlock.badge')}</CardBadge>
...
export function GoalSummaryCard({ shape }: CardProps) {
  ...
  const title = String(p.title ?? t('cards.goalSummary.defaultTitle'))
  ...
  return (
    <div className={`${cardBase} border-cyan-400/20 bg-gradient-to-b from-cyan-400/[0.08] to-[#0a0e16]/95 p-5`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="flex h-full flex-col justify-between">
        <div>
          <CardBadge className="border-cyan-400/20 bg-cyan-400/10 text-cyan-300">{t('cards.goalSummary.badge')}</CardBadge>
          <div className="mt-3 text-lg font-bold tracking-tight text-white">{title}</div>
        </div>
        <div>
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
            <span className="text-slate-400">{t('cards.goalSummary.tasks')} <span className="font-semibold text-white">{taskCount}</span></span>
            {executingCount > 0 && <span className="text-emerald-400">{t('cards.goalSummary.running', { count: executingCount })}</span>}
            {failedCount > 0 && <span className="text-red-400">{t('cards.goalSummary.failed', { count: failedCount })}</span>}
            {skippedCount > 0 && <span className="text-slate-500">{t('cards.goalSummary.skipped', { count: skippedCount })}</span>}
            <span className="text-cyan-400">{t('cards.goalSummary.done', { count: completedCount })}</span>
          </div>
...
export function ErrorCardComponent({ shape }: CardProps) {
  ...
  const { message, stack = '', severity = 'error' } = shape.props as Record<string, string>
  const displayMessage = message || t('cards.errorCard.defaultMessage')
    ...
    <div className={`${cardBase} ${cardCls}`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="p-4">
        <CardBadge className={badgeCls}>
          {isWarning ? t('cards.errorCard.badgeWarning') : t('cards.errorCard.badgeError')}
        </CardBadge>
        <div className="mt-3 text-sm font-semibold leading-snug text-red-200">{displayMessage}</div>
        {stack && (
          <>
            <button
              type="button"
            ...
            >
              {expanded ? t('cards.errorCard.hideStackTrace') : t('cards.errorCard.showStackTrace')}
            </button>
            {expanded && (
  ...
  return (
    <div className={`${cardBase} ${allPassed ? 'border-emerald-400/15 bg-gradient-to-b from-emerald-400/[0.06]' : 'border-red-400/15 bg-gradient-to-b from-red-400/[0.06]'} to-[#0a0e16]/95 p-4`} style={{ width: shape.w, height: shape.h }}>
      ...
      <CardBadge className={allPassed ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300' : 'border-red-400/20 bg-red-400/10 text-red-300'}>
        {allPassed ? t('cards.testResult.badgePassed') : t('cards.testResult.badgeFailed')}
      </CardBadge>
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-lg bg-emerald-400/[0.06] border border-emerald-400/10 px-2.5 py-2 text-center">
          ...
          <div className="text-emerald-400/60 text-[10px] font-semibold uppercase">{t('cards.testResult.passed')}</div>
        </div>
        <div className="rounded-lg bg-red-400/[0.06] border border-red-400/10 px-2.5 py-2 text-center">
          ...
          <div className="text-red-400/60 text-[10px] font-semibold uppercase">{t('cards.testResult.failed')}</div>
        </div>
      </div>
      {coverage > 0 && (
        <div className="mt-2 flex items-center justify-between text-xs text-slate-400">
          <span>{t('cards.testResult.coverage')}</span>
...
export function LinkCardComponent({ shape }: CardProps) {
  const { t } = useTranslation('canvas')
  const { url = '', title, description = '' } = shape.props as Record<string, string>
  const displayTitle = title || t('cards.linkCard.defaultTitle')
  return (
    <div className={`${cardBase} border-blue-400/15 bg-gradient-to-b from-blue-400/[0.04] to-[#0a0e16]/95 p-4`} style={{ width: shape.w, height: shape.h }}>
      ...
      <div className="flex h-full flex-col justify-between">
        <div>
          <CardBadge className="border-blue-400/20 bg-blue-400/10 text-blue-300">{t('cards.linkCard.badge')}</CardBadge>
          <div className="mt-3 text-sm font-semibold tracking-tight text-white">{displayTitle}</div>
...
export function MetricCardComponent({ shape }: CardProps) {
  const { t } = useTranslation('canvas')
  const p = shape.props as Record<string, unknown>
  const label = String(p.label ?? t('cards.metricCard.defaultLabel'))
  ...
  return (
    <div className={`${cardBase} border-purple-400/15 bg-gradient-to-b from-purple-400/[0.04] to-[#0a0e16]/95 p-4`} style={{ width: shape.w, height: shape.h }}>
      <AiBadge source={shape.source} />
      <div className="flex h-full flex-col justify-between">
        <CardBadge className="border-purple-400/20 bg-purple-400/10 text-purple-300">{t('cards.metricCard.badge')}</CardBadge>
