...
Strada 现在还会为每个任务维护内部 execution journal 和 rollback memory。replan 可以复用最近的稳定 checkpoint、记住已经耗尽的 branch，并把 adaptive phase scores 回灌到 routing，而不依赖 hardcoded provider lore。
...
| `/routing info` | 显示当前身份最近的路由决策、runtime execution traces、phase outcomes 以及 adaptive phase scores，覆盖 planning、execution、clarification-review、review、synthesis 阶段 |
