...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-3100%2B-brightgreen?style=flat-square" alt="测试">
...
**贝叶斯置信度评分：**
- 本能使用 **Beta 后验推断**（`confidence = alpha / (alpha + beta)`），以 `Beta(1,1)` 无信息先验为基础
- 评定分数（0.0-1.0）作为分数证据权重进行精细更新
- 无混合或时间衰减——纯贝叶斯后验均值
...
src/
  ...
  learning/
    ...
    scoring/
      confidence-scorer.ts  # 贝叶斯 Beta 后验置信度、Elo、Wilson 区间
    matching/
