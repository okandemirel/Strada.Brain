...
<p align="center">
  ...
  一个连接到 Web 仪表板、Telegram、Discord、Slack、WhatsApp 或终端的自主编码代理 &mdash; 读取您的代码库、编写代码、运行构建、从错误中学习，并通过 24/7 守护进程循环实现自主运行。现已支持多代理编排、任务委派、记忆整合、带审批门控的部署子系统，以及支持 LLM 视觉识别的媒体共享。
</p>

<p align="center">
  <img src="https://img.shields.io/badge/TypeScript-5.7-blue?style=flat-square&logo=typescript" alt="TypeScript">
  <img src="https://img.shields.io/badge/Node.js-%3E%3D20-green?style=flat-square&logo=node.js" alt="Node.js">
  <img src="https://img.shields.io/badge/tests-3220%2B-brightgreen?style=flat-square" alt="测试">
...
| `EMBEDDING_DIMENSIONS` | `256` | 嵌入向量维度（支持 Matryoshka 维度：256、512、768、1024） |
...
3. 通过 Gemini Embedding 2.0（默认，支持 Matryoshka 维度）、OpenAI（`text-embedding-3-small`）、Mistral、Together、Fireworks、Qwen 或 Ollama（`nomic-embed-text`）生成嵌入向量
...
每秒推送的实时指标。支持认证连接和远程命令（插件重载、缓存清除、日志检索）。守护进程事件（触发器触发、预算警告、目标进度）通过 WebSocket 广播。
