...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4527%2B-brightgreen?style=flat-square" alt="测试">
...
它拥有基于 SQLite + HNSW 向量的持久记忆，通过混合加权置信度评分从过去的错误中学习，将复杂目标分解为并行 DAG 执行，自动合成多工具链并支持 saga 回滚，并可作为 24/7 守护进程运行，支持主动触发器。它支持多代理编排（按通道/会话隔离）、跨代理层级的任务委派、自动记忆整合、带人工审批门控和断路器保护的部署子系统，以及基于 Magic UI 组件（shadcn/ui + 21st.dev）的现代毛玻璃 Web 门户，具备动画指标、模糊过渡和持久通知中心。
...
当 Strada 通过打包后的 npm/tarball 版本安装时，它会默认把运行时配置保存在 `~/.strada`（macOS/Linux）或 `%LOCALAPPDATA%\Strada`（Windows），而不是依赖当前工作目录。如果你需要不同的 app home，可以使用 `STRADA_HOME=/custom/path` 覆盖它。
...
在 Windows 上，同一流程会优先使用 `nvm-windows`，其次使用 `winget`，最后才退回到直接下载 Node。此时显示的重跑命令会是 `.\strada.ps1 setup --web`。
...
已配置的 `UNITY_PROJECT_PATH` 是 Strada 进行代码工作的权威项目范围。即使编辑器里当前打开的是另一个 Unity 项目，Strada 也只会在启动时提示 mismatch 警告，不会悄悄切换离开 setup 中选定的项目。
新的 setup 现在默认同时写入 `MULTI_AGENT_ENABLED=true` 和 `TASK_DELEGATION_ENABLED=true`。如果你想走 legacy 单代理路径，请显式设置 `MULTI_AGENT_ENABLED=false`；即使 `TASK_DELEGATION_ENABLED=true`，只要 multi-agent 关闭，委派也不会初始化。
...
对于 git/source 安装，只要 source launcher 已经可用，`strada doctor` 就不会再把缺失的 `dist/` 当成阻塞错误。它会给出 warning，并且只在你确实需要打包产物时提示准确的 repo-root `npm run bootstrap` 命令。
...
.\strada.ps1             # Windows PowerShell 的 source launcher
strada.cmd               # Windows Command Prompt companion launcher
./strada install-command  # 为用户安装 bare `strada` 命令
./strada uninstall        # 移除已安装的 bare 命令以及托管的 PATH/profile 改动
.\strada.ps1 uninstall   # 从 Windows checkout 执行同样的卸载
strada uninstall --purge-config # 同时移除 Strada 创建的 repo-local runtime state
...
.\strada.ps1 setup --web # 直接从 PowerShell 打开 Web 向导
.\strada.ps1 setup --terminal # 直接从 PowerShell 打开终端向导
.\strada.ps1 doctor      # 从 PowerShell 运行 doctor
...
Strada.Brain 每天自动检查更新，在空闲时应用更新。源码 checkout 和 `./strada install-command` 安装会通过 git 更新。git 更新成功后，Strada 还会重写已安装 bare command 的 wrapper，使 `strada` 继续指向当前 checkout。基于 npm 的更新命令只有在 public npm release 存在时才适用。
...
**技术栈：** React 19、Vite、Tailwind CSS v4、shadcn/ui、Zustand、TanStack Query、Radix UI、ReactFlow、tldraw、Monaco Editor。

### 基于 Shadcn/UI + 21st.dev 的现代界面

Web 门户采用由 shadcn/ui 以及 [21st.dev](https://21st.dev) 和 [Magic UI](https://magicui.design) 社区组件驱动的高级毛玻璃设计：

- **毛玻璃主题**：所有面板均采用 `backdrop-blur`、半透明表面、光晕效果和微交互
- **Magic UI 组件**：NumberTicker（动画指标）、BlurFade（页面过渡）、BorderBeam（活跃卡片）、TypingAnimation（"思考中..."指示器）、SparklesText（品牌星光）、ShimmerButton（发送闪光）、CoolMode（纸屑彩蛋）
- **Sonner 通知**：具有自动消失、严重性颜色、撤销操作和持久通知中心抽屉的 Toast 系统
- **可折叠管理导航**：支持路由感知自动展开，从侧边栏可访问 11 个管理页面
- **共享基础组件**：PageSkeleton、PageError、CopyButton、Sparkline、StatusDot、Badge、Sheet、ScrollArea、Input、Table
...
**目标执行（通过 SupervisorDispatcher）：**
- 基于波次的并行执行，遵循依赖排序
- 基于信号量的并发限制（`SUPERVISOR_MAX_PARALLEL_NODES`）
- 失败预算（`SUPERVISOR_MAX_FAILURE_BUDGET`），支持可配置阈值
- 每节点超时，AbortSignal 传播至 fetch 层
- 通过 ProviderHealthRegistry 实现健康感知的提供者回退
...
新 setup 会默认写入 `TASK_DELEGATION_ENABLED=true`，但只有在 `MULTI_AGENT_ENABLED=true` 时委派才会初始化。
...
- **观察**：从已注册的观察器集合收集环境状态。当前默认 daemon 接线使用 trigger、user-activity 和 git-state 观察器；build/test 观察器只会在这些 runtime 信号可用时接入
...
权限边界：Agent Core 是主动的目标生成器和通知层，不是 PAOR 执行器的并行替代。interactive/background 执行仍然通过 orchestrator 的 PAOR 循环、verifier pipeline 和共享 loop-recovery 控制来完成。
