...
5. **计划阶段** — LLM 根据已学习的洞察和过去的失败创建编号计划
6. **行动阶段** — LLM 按照计划执行工具调用
7. **观察** — 记录结果；错误恢复分析失败；故障分类器对错误进行分类
8. **反思** — 每 3 步（或出错时），LLM 决定：**继续**、**重新规划** 或 **完成**
9. **自动重新规划** — 如果连续 3 次以上相同类型的失败，强制采用避免失败策略的新方法
10. **重复** 最多 50 次迭代直到完成
11. **发送响应** — 通过频道发送给用户（如支持则以流式传输）
...
**主动检索（新功能）：** 每个任务开始时，通过 `InstinctRetriever` 主动查询本能。它使用关键词相似性和 HNSW 向量嵌入搜索相关的已学习模式，并将其注入计划阶段的提示中。

**任务分解：** 复杂的多步骤请求通过启发式分析自动检测，并在执行前通过 LLM 分解为 3-8 个有序子任务。
...
110 个测试文件覆盖：代理、频道、安全、RAG、记忆、学习、仪表板、集成流程。
...
src/
  ...
  learning/             # 模式匹配、置信度评分、本能生命周期、HNSW 语义搜索
