...
Strada ahora tambien mantiene un execution journal y rollback memory internos por tarea. Los replans pueden reutilizar el ultimo checkpoint estable, recordar ramas agotadas y devolver adaptive phase scores al routing sin lore hardcodeado por proveedor. Esos scores ahora tambien consideran verifier clean rate, rollback pressure, retry count, repeated failure fingerprints y phase-local token cost.
La memoria ahora tambien se separa por rol: user profile state guarda nombre/preferencias/autonomy, task execution memory guarda session summaries/open items/rollback state, y project/world memory ahora se inyecta explicitamente desde el project root activo y la AgentDB analysis cacheada, mientras semantic retrieval sigue agregando memoria relevante en vivo por separado.
