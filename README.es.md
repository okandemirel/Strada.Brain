...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-3300%2B-brightgreen?style=flat-square" alt="Pruebas">
...
Strada ahora tambien mantiene un execution journal y rollback memory internos por tarea. Los replans pueden reutilizar el ultimo checkpoint estable, recordar ramas agotadas, llevar un project/world anchor y devolver adaptive phase scores al routing sin lore hardcodeado por proveedor. Esos scores ahora tambien consideran verifier clean rate, rollback pressure, retry count, repeated failure fingerprints, repeated world-context failures y phase-local token cost.
La memoria ahora tambien se separa por rol: user profile state guarda nombre/preferencias/autonomy, task execution memory guarda session summaries/open items/rollback state, y project/world memory ahora se inyecta explicitamente desde el project root activo y la AgentDB analysis cacheada. Esa misma capa project/world ahora tambien alimenta recovery memory y adaptive routing, mientras semantic retrieval sigue agregando memoria relevante en vivo por separado.
...
La correlacion del replay ahora tambien se persiste con `taskRunId` scoped al chat, para que tareas concurrentes dentro del mismo chat no mezclen phase telemetry ni recovery history.
...
| `/routing info` | Mostrar decisiones de enrutamiento recientes, trazas de ejecucion reales, phase outcomes y adaptive phase scores para la identidad actual, incluyendo verifier clean rate, rollback pressure, retry count y token-cost telemetry en planning, execution, clarification-review, review y synthesis |
