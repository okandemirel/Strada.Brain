...
Strada conserve d&eacute;sormais aussi un execution journal et une rollback memory internes pour chaque t&acirc;che. Les replans peuvent r&eacute;utiliser le dernier checkpoint stable, m&eacute;moriser les branches &eacute;puis&eacute;es et renvoyer des adaptive phase scores vers le routing sans lore de fournisseur hardcod&eacute;. Ces scores tiennent maintenant aussi compte du verifier clean rate, du rollback pressure, du retry count, des repeated failure fingerprints et du phase-local token cost.
La m&eacute;moire est maintenant s&eacute;par&eacute;e par r&ocirc;le : user profile state garde nom/pr&eacute;f&eacute;rences/autonomy, task execution memory garde session summaries/open items/rollback state, et project/world memory reste dans AgentDB analysis plus semantic retrieval.
