...
Quand Strada est installe depuis une version npm/tarball empaquetee, sa configuration runtime est stockee par defaut dans `~/.strada` au lieu de dependre du dossier courant. Utilisez `STRADA_HOME=/chemin/personnalise` si vous voulez un autre app home.
...
Strada.Brain v&eacute;rifie automatiquement les mises &agrave; jour quotidiennement et les applique en cas d'inactivit&eacute;. Les checkouts source et les installations `./strada install-command` se mettent &agrave; jour via git. Les commandes de mise &agrave; jour bas&eacute;es sur npm ne s'appliquent qu'une fois une publication npm publique disponible.
...
Strada conserve d&eacute;sormais aussi un execution journal et une rollback memory internes pour chaque t&acirc;che. Les replans peuvent r&eacute;utiliser le dernier checkpoint stable, m&eacute;moriser les branches &eacute;puis&eacute;es, transporter un project/world anchor et renvoyer des adaptive phase scores vers le routing sans lore de fournisseur hardcod&eacute;. Ces scores tiennent maintenant aussi compte du verifier clean rate, du rollback pressure, du retry count, des repeated failure fingerprints, des repeated world-context failures et du phase-local token cost.
La m&eacute;moire est maintenant s&eacute;par&eacute;e par r&ocirc;le : user profile state garde nom/pr&eacute;f&eacute;rences/autonomy, task execution memory garde session summaries/open items/rollback state, et project/world memory est maintenant inject&eacute; explicitement &agrave; partir du project root actif plus la AgentDB analysis en cache. Cette m&ecirc;me couche project/world alimente maintenant aussi la recovery memory et l'adaptive routing, pendant que semantic retrieval ajoute toujours la m&eacute;moire pertinente en direct s&eacute;par&eacute;ment.
...
La corr&eacute;lation du replay est maintenant aussi persist&eacute;e avec un `taskRunId` scoped au chat, afin que des t&acirc;ches concurrentes dans le m&ecirc;me chat ne m&eacute;langent pas leur phase telemetry ni leur recovery history.
...
| `/routing info` | Afficher les d&eacute;cisions de routage r&eacute;centes, les traces d'ex&eacute;cution r&eacute;elles, les phase outcomes et les adaptive phase scores pour l'identit&eacute; active, avec verifier clean rate, rollback pressure, retry count et token-cost telemetry sur planning, execution, clarification-review, review et synthesis |
