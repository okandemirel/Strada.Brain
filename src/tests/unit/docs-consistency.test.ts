...
describe("docs consistency", () => {
  ...
  it("marks localized READMEs as translations that defer to the English canonical README", () => {
    for (const relativePath of localizedReadmes) {
      ...
      expect(content).toContain("strada uninstall");
      expect(content).toContain("--purge-config");
      ...
      expect(content).toContain(".\\Strada.Brain\\strada.ps1 install-command");
      expect(content).toContain(".\\Strada.Brain\\strada.ps1 setup");
      expect(content).toContain(".\\strada.ps1");
      expect(content).toContain("strada.cmd");
      expect(content).toMatch(/\.\\strada\.ps1 setup --web/u);
      expect(content).toMatch(/\.\\strada\.ps1 setup --terminal/u);
      expect(content).toMatch(/\.\\strada\.ps1 doctor/u);
      expect(content).toMatch(/\.\\strada\.ps1 uninstall/u);
      expect(content).toContain("%LOCALAPPDATA%\\Strada");
      expect(content).toContain("STRADA_HOME");
  ...
  it("keeps install docs aligned with the source-first release path", () => {
    ...
    expect(englishReadme).toContain("strada uninstall");
    expect(englishReadme).toContain("--purge-config");
    expect(englishReadme).toContain(".\\strada.ps1 uninstall");
    ...
    expect(englishReadme).toContain("%LOCALAPPDATA%\\Strada");
    expect(englishReadme).toContain("STRADA_HOME");
    expect(englishReadme).toContain("./strada setup --web");
    expect(englishReadme).toContain(".\\strada.ps1 setup --web");
    expect(englishReadme).toContain("strada.cmd");
    expect(englishReadme).toContain("Node.js 20.19+");
    expect(englishReadme).toContain("nvm");
    expect(englishReadme).toContain("winget");
    ...
    expect(englishReadme).toMatch(/bare-command wrappers/iu);
