...
function renderSetupStatusHtml(status: SetupStatusResponse): string {
  const heading = status.state === "failed"
    ...
    : "Configuration saved";
  const primary = getSetupStatusDetail(status);
...
export function buildSetupEnvLines(
  ...
  const lines: string[] = [
    ...
    `UNITY_PROJECT_PATH=${formatEnvValue(validatedProjectPath)}`,
  ];
  // Written only when this save states it: the key is the operator's consent to
  // load a project-local Strada.MCP in-process (COR-12), never a default.
  if (config.STRADA_MCP_ALLOW_PROJECT_LOCAL === "true" || config.STRADA_MCP_ALLOW_PROJECT_LOCAL === "false") {
    lines.push(`STRADA_MCP_ALLOW_PROJECT_LOCAL=${config.STRADA_MCP_ALLOW_PROJECT_LOCAL}`);
  }

  for (const key of PROVIDER_ENV_KEYS) {
    if (config[key]) lines.push(`${key}=${formatEnvValue(config[key])}`);
  }
  if (config.OPENAI_AUTH_MODE) {
    lines.push(`OPENAI_AUTH_MODE=${formatEnvValue(config.OPENAI_AUTH_MODE)}`);
  }
  if (config.ANTHROPIC_AUTH_MODE) {
    lines.push(`ANTHROPIC_AUTH_MODE=${formatEnvValue(config.ANTHROPIC_AUTH_MODE)}`);
  }
  if (config.OPENAI_CHATGPT_AUTH_FILE) {
    lines.push(`OPENAI_CHATGPT_AUTH_FILE=${formatEnvValue(config.OPENAI_CHATGPT_AUTH_FILE)}`);
  }

  const opencodeBaseUrl = config.OPENCODE_BASE_URL?.trim();
  if (opencodeBaseUrl && HTTP_URL_RE.test(opencodeBaseUrl)) {
    lines.push(`OPENCODE_BASE_URL=${formatEnvValue(opencodeBaseUrl)}`);
  }
  ...
  if (opencodeDefaultModel && MODEL_NAME_RE.test(opencodeDefaultModel)) {
    lines.push(`OPENCODE_DEFAULT_MODEL=${formatEnvValue(opencodeDefaultModel)}`);
  }

  if (config.PROVIDER_CHAIN) {
    lines.push(`PROVIDER_CHAIN=${formatEnvValue(config.PROVIDER_CHAIN)}`);
  }

  for (const [provider, model] of buildProviderModelDefaults(config)) {
    lines.push(`${provider.toUpperCase()}_MODEL=${formatEnvValue(model)}`);
  }
  ...
  if (config.RAG_ENABLED === "false") {
    ...
    lines.push("", "# RAG (Code Search)", `EMBEDDING_PROVIDER=${formatEnvValue(config.EMBEDDING_PROVIDER)}`);
    const embeddingModel = typeof config.EMBEDDING_MODEL === "string" ? config.EMBEDDING_MODEL.trim() : "";
    if (embeddingModel) {
      lines.push(`EMBEDDING_MODEL=${formatEnvValue(embeddingModel)}`);
    }
  }
  ...
  const lang = config.LANGUAGE_PREFERENCE && KNOWN_LANGUAGES.has(String(config.LANGUAGE_PREFERENCE))
    ...
    : "en";
  lines.push("", "# Language", `LANGUAGE_PREFERENCE=${formatEnvValue(lang)}`);
  ...
  if (hasGeminiKey && !hasExplicitEmbedding && !ragDisabled) {
    lines.push("", "# Gemini offers free embedding with excellent quality", `EMBEDDING_PROVIDER=${formatEnvValue("gemini")}`);
  }
  ...
  if (config.SYSTEM_PRESET && KNOWN_PRESETS.has(String(config.SYSTEM_PRESET))) {
    lines.push("", "# System Preset", `SYSTEM_PRESET=${formatEnvValue(String(config.SYSTEM_PRESET))}`);
  }

  if (config._channel && KNOWN_CHANNELS.has(String(config._channel))) {
    lines.push("", "# Channel", `DEFAULT_CHANNEL=${formatEnvValue(String(config._channel))}`);
  }

  lines.push(
    "",
    "# Web Channel",
    `WEB_CHANNEL_PORT=${port}`,
    "DASHBOARD_PORT=3100",
  );
  ...
  if (config.OBSIDIAN_ENABLED === "true" || config.OBSIDIAN_ENABLED === "false") {
    ...
    if (config.OBSIDIAN_ENABLED === "true") {
      ...
      if (vaultPath) lines.push(`OBSIDIAN_VAULT_PATH=${formatEnvValue(vaultPath)}`);
      const apiKey = typeof config.OBSIDIAN_API_KEY === "string" ? config.OBSIDIAN_API_KEY.trim() : "";
      if (apiKey) lines.push(`OBSIDIAN_API_KEY=${formatEnvValue(apiKey)}`);
    }
  }
...
export class SetupWizard {
  ...
  /**
   * Projects the operator installed Strada.MCP into through this wizard. That
   * install is the operator choosing to run the copy, so the save that follows
   * records the consent a project-local copy needs to load (COR-12).
   */
  ...
  private resetToCollecting(): void {
    this.status = transitionSetupStatus(this.status, { type: "reset" });
  }
  ...
  private async handleInstallMcp(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    this.mcpInstalledInProject.add(validatedProjectPath.resolved);
  ...
  private async handleSaveConfig(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    let providerWarnings: ResponseProviderPreflightFailure[] | undefined;
    ...
    // RAG configuration
    if (config.EMBEDDING_PROVIDER && config.EMBEDDING_PROVIDER !== "auto") {
    ...
    if (
      config.STRADA_MCP_ALLOW_PROJECT_LOCAL === undefined &&
      this.mcpInstalledInProject.has(validatedProjectPath.resolved)
    ) {
      config.STRADA_MCP_ALLOW_PROJECT_LOCAL = "true";
    }
    let lines: string[];
    try {
      lines = buildSetupEnvLines(config, validatedProjectPath.resolved, this.port);
    } catch (err) {
      // formatEnvValue refuses a value dotenv could not read back unchanged.
      this.json(res, 400, { success: false, error: err instanceof Error ? err.message : String(err) });
      return;
    }
