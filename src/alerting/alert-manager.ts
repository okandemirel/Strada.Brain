/**
 * Alert Manager for Strada.Brain
...
export class AlertManager {
  ...
  async sendAlert(
    ...
    if (this.isRateLimited(alertKey)) {
      try {
        getLogger().info(`[AlertManager] Rate limited: ${alertKey}`);
      } catch {
  ...
  private async sendDiscord(alert: Alert): Promise<void> {
    ...
    const embed: DiscordEmbed = {
      ...
      fields: [],
      footer: { text: `Strada.Brain Alert • ID: ${alert.id}` },
    };
    ...
    if (alert.metadata.hostname) {
      embed.fields!.push({
        name: "Hostname",
        value: String(alert.metadata.hostname),
        inline: true,
      });
    }
      ...
      attachments: [
        {
          ...
          fields: [
            ...
            ...(alert.metadata.source
              ? [{ title: "Source", value: String(alert.metadata.source), short: true }]
              : []),
            ...(alert.metadata.hostname
              ? [{ title: "Hostname", value: String(alert.metadata.hostname), short: true }]
              : []),
          ],
          footer: `Strada.Brain Alert • ${alert.id}`,
    ...
    const response = await fetch("https://events.pagerduty.com/v2/enqueue", {
      ...
      body: JSON.stringify({
        ...
        payload: {
          ...
          source: alert.metadata.source || "Strada.Brain",
          custom_details: {
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>This alert was generated by Strada.Brain Alert Manager</p>
        </div>
    </div>
</body>
</html>
  ...
  private generateAlertId(): string {
    return createHash("sha256").update(randomBytes(16)).digest("hex").slice(0, 12);
  }
  ...
  private recordAlert(alert: Alert, duration: number): void {
    ...
    this.stats.averageResponseTime =
      (this.stats.averageResponseTime * (this.stats.totalAlerts - 1) + duration) /
      this.stats.totalAlerts;
  }
