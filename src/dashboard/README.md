...
- `POST /api/consolidation/run` and `POST /api/deployment/check` (a consolidation cycle calls the LLM per cluster; the readiness check runs the project's test command) can outlast one HTTP answer, so they start a job and answer **202** `{ jobId, kind, state: "running", startedAt }` at once. Only one job per kind runs: a second start answers **409** `{ error, jobId }` naming the running one. `deployment/check` without deployment configured still answers 200 `{ enabled: false }` at once.
- `GET /api/daemon/jobs/:id` answers `{ id, kind, state: "running" | "done" | "failed", startedAt, finishedAt?, result?, error? }`, or 404. It is an ordinary read: the dashboard bearer gates it, and the operator token does not open it. Jobs live in memory for this run of the dashboard, at most 50 at once; finished ones expire after an hour. The CLI polls it every 1.5 s with a one-line progress indicator; Ctrl-C stops the waiting, not the job, and `strada daemon job <id>` shows it later.
