...
describe("InstinctRetriever", () => {
  ...
  // audited 2026-09-02: seeds, teachExplicit, recordCorrection and
  // recordAutoResolution all store `action` as plain prose. The retriever
  // used to JSON.parse it, drop it on throw, and still credit the id.
  describe("plain-text actions (the production majority)", () => {
    it("renders a prose action as its own description instead of dropping it", async () => {
      const taught = createMockInstinct({
        id: "instinct_taught" as Instinct["id"],
        triggerPattern: "generated assets",
        action: "never edit Assets/Generated by hand",
        confidence: 0.9,
        stats: {
          timesSuggested: 0,
          timesApplied: 0,
      ...
      const seeded = createMockInstinct({
        id: "instinct_seed" as Instinct["id"],
        triggerPattern: "dependency injection",
        action: "Use Strada.Core DI container, not Zenject/VContainer",
        confidence: 0.8,
      });
      const jsonControl = createMockInstinct({
        id: "instinct_json" as Instinct["id"],
        triggerPattern: "json control",
        action: JSON.stringify({ description: "Valid insight" }),
        confidence: 0.7,
      });

      const { retriever } = setup(
        [taught, seeded, jsonControl],
        [createMockMatch(taught, 0.9), createMockMatch(seeded, 0.85), createMockMatch(jsonControl, 0.7)],
      );

      const result = await retriever.getInsightsForTask("some task");

      expect(result.insights).toHaveLength(3);
      expect(result.insights[0]).toContain("never edit Assets/Generated by hand");
    ...
    it("credits an id only when an insight for it actually reached the caller", async () => {
      const good = createMockInstinct({
        id: "instinct_good" as Instinct["id"],
        triggerPattern: "valid",
        action: JSON.stringify({ description: "Valid insight" }),
        confidence: 0.9,
      });
      // Empty action: nothing can be rendered, so nothing may be credited.
      const empty = createMockInstinct({
        id: "instinct_empty" as Instinct["id"],
      ...
      const { retriever } = setup(
        [good, empty],
        [createMockMatch(good, 0.9), createMockMatch(empty, 0.85)],
      );

      const result = await retriever.getInsightsForTask("some task");

      expect(result.insights).toHaveLength(1);
      // ids and insights stay index-aligned (memory-refresher zips them)
      expect(result.matchedInstinctIds).toEqual(["instinct_good"]);
    });
  });
