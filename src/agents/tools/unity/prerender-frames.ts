...
export interface PrerenderStyle {
  ...
  bodyColor?: string;
  /**
   * The profile's shading. It decides the material's gloss and metalness;
   * every character used to be rendered glossy whatever the document asked
   * for (Codex 2026-09-11 B#21).
   */
  shading?: string;
...
/** Gloss and metalness per shading family; unknown shading is matte. */
export function materialForShading(shading: string | undefined): { glossiness: number; metallic: number } {
  switch ((shading ?? "").toLowerCase()) {
    case "glossy":
      return { glossiness: 0.82, metallic: 0 };
    case "pbr-realistic":
      return { glossiness: 0.5, metallic: 0.1 };
    case "unlit":
      return { glossiness: 0, metallic: 0 };
    case "flat":
    default:
      return { glossiness: 0.1, metallic: 0 };
  }
}
...
export function buildRenderScript(style: Required<PrerenderStyle>): string {
  ...
  const customBody = style.bodyColor !== undefined && style.bodyColor !== "" ? hexToRgb(style.bodyColor, neutral) : body;
  const { glossiness, metallic } = materialForShading(style.shading);
  ...
  SubShader {
    ...
    Pass {
      ...
      ENDCG
        ...
        {
            bool isEye = ${glossiness >= 0.8 ? "smr.name.ToLowerInvariant().Contains(\"eye\")" : "false"};
...
export class PrerenderFramesTool implements ITool {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext): Promise<ToolExecutionResult> {
    ...
    let styleProfileDefaults: { bodyColor?: string; outlineWidth?: number; plump?: number; headScale?: number; shading?: string } = {};
    try {
