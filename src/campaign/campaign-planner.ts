...
- Every element the GDD schedules must end its milestone with a real, BOUND visual: source it with unity_my_assets first, generate it with unity_generate_sprite when nothing fits, and bind it into the element's prefab. Code for an element without its sprite is a milestone that is not done.
