...
| provider concurrency       | numeric field              | `PROVIDER_MAX_CONCURRENT_REQUESTS` (default 3, range 1..32)                                                                                                                                                                                                                                                             |
...
- Setup now writes both `MULTI_AGENT_ENABLED=true` and `TASK_DELEGATION_ENABLED=true` by default. Delegation remains gated by multi-agent initialization, so `TASK_DELEGATION_ENABLED=true` has no runtime effect when `MULTI_AGENT_ENABLED=false`.
