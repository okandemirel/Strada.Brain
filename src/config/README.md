...
| AI providers               | top-level fields           | `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_MODE`, `ANTHROPIC_AUTH_TOKEN`, `OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `GEMINI_API_KEY`, etc. At least one usable hosted credential is required unless `PROVIDER_CHAIN` only uses local providers such as `ollama`; Claude can also authenticate via `ANTHROPIC_AUTH_MODE=claude-subscription` plus a bearer token generated from the local Claude CLI login, and OpenAI can authenticate via `OPENAI_AUTH_MODE=chatgpt-subscription` plus the local Codex auth session file. |
...
| `websocketDashboard`       | `WebSocketDashboardConfig` | `ENABLE_WEBSOCKET_DASHBOARD`, `WEBSOCKET_DASHBOARD_PORT` (default 3101; must differ from `DASHBOARD_PORT` when both dashboards are enabled), `WEBSOCKET_DASHBOARD_AUTH_TOKEN`, `WEBSOCKET_DASHBOARD_ALLOWED_ORIGINS` (`WEBSOCKET_DASHBOARD_AUTH_TOKEN` also protects dashboard APIs when present)                                                                                            |
...
| `bindHost`                 | string                     | `BIND_HOST` (default 127.0.0.1; the address every HTTP listener binds to — set 0.0.0.0 in a container so a published port is reachable)                                                                                                                                                                                                                                                                                        |
...
| provider concurrency       | numeric field              | `PROVIDER_MAX_CONCURRENT_REQUESTS` (default 3, range 1..20)                                                                                                                                                                                                                                                             |
| `agent`                    | `AgentConfig`              | `MULTI_AGENT_ENABLED` (default false; the setup wizard writes `true`), `AGENT_DEFAULT_BUDGET_USD`, `AGENT_MAX_CONCURRENT`, `AGENT_IDLE_TIMEOUT_MS`, `AGENT_MAX_MEMORY_ENTRIES`                                                                                                                                                                            |
| `delegation`               | `DelegationConfig`         | `TASK_DELEGATION_ENABLED` (default false; the setup wizard writes `true`), `AGENT_MAX_DELEGATION_DEPTH`, `AGENT_MAX_CONCURRENT_DELEGATIONS`, `DELEGATION_TIER_LOCAL`, `DELEGATION_TIER_CHEAP`, `DELEGATION_TIER_STANDARD`, `DELEGATION_TIER_PREMIUM`, `DELEGATION_VERBOSITY`, `DELEGATION_TYPES`                    |
...
- Setup now writes both `MULTI_AGENT_ENABLED=true` and `TASK_DELEGATION_ENABLED=true` by default. The schema itself prefaults both to `false`, so a hand-rolled `.env` (or one copied from `.env.example`, where both keys are commented out) boots with multi-agent and delegation off. Delegation remains gated by multi-agent initialization, so `TASK_DELEGATION_ENABLED=true` has no runtime effect when `MULTI_AGENT_ENABLED=false`.
- `ANTHROPIC_AUTH_MODE` - `api-key` (default) or `claude-subscription`; when set to subscription mode Strada uses `ANTHROPIC_AUTH_TOKEN` bearer auth for Claude conversation turns instead of the Anthropic platform API key
- `ANTHROPIC_AUTH_TOKEN` - Claude subscription bearer token generated after `claude auth login --claudeai` and `claude setup-token`
...
- Claude subscription auth is bearer-token based. It does not reuse the Claude CLI session directly; setup and manual config require `ANTHROPIC_AUTH_TOKEN`.
- Anthropic documents claude.ai subscription auth as limited to Claude Code and Claude.ai. Strada still exposes `claude-subscription` as an advanced, user-assumed-risk option.
- Setup and `strada doctor` surface guidance to run `claude auth login --claudeai` and `claude setup-token` when Claude subscription mode is selected without a token, and they still require the configured Claude response worker to pass real preflight before treating it as usable.
...
The credential gate that is actually enforced at boot lives in the schema (`configSchema.superRefine` in `config-schema.ts`, run by `validateConfig`): at least one provider key or subscription, or `ollama` in the effective provider chain (`SYSTEM_PRESET` is resolved before validation, so the `free` preset's chain satisfies it). The helpers below are exported library functions and are **not called by `loadConfig`** — importing them is the only way they run.

- `validateProjectPath(path)` resolves symlinks via `realpathSync`, checks `isDirectory()`, returns `Result<string, string>` (this one `loadConfig` does call)
- `hasRequiredApiKeys(config)` checks that at least one usable provider is configured, or that every provider named in `PROVIDER_CHAIN` has its required key/subscription auth — not invoked by `loadConfig`
- `checkChannelConfig(config, channelType)` validates channel-specific requirements — not invoked by `loadConfig`:
