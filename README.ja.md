...
Strada は各タスクについて内部 execution journal と rollback memory も保持するようになりました。replan では最後の安定 checkpoint や行き詰まった branch を再利用でき、hardcoded な provider lore なしで adaptive phase scores を routing に戻します。これらの score は verifier clean rate、rollback pressure、retry count、repeated failure fingerprints、phase-local token cost も考慮します。
メモリも役割ごとに分離されました。user profile state は名前/設定/autonomy、task execution memory は session summaries/open items/rollback state を保持し、project/world memory はアクティブな project root と cached AgentDB analysis から explicit な prompt layer として注入されます。semantic retrieval は引き続き live な関連 memory を別で追加します。
