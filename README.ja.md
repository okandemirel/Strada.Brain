...
| `/routing info` | 現在のIDに対する最近のルーティング決定、runtime execution traces、phase outcomes、adaptive phase scores を表示（verifier clean rate / rollback pressure / retry count / token-cost telemetry / provider catalog freshness / official alignment / capability drift を含み、planning / execution / clarification-review / review / synthesis を対象にする） |
