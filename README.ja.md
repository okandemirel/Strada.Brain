...
メモリも役割ごとに分離されました。user profile state は名前/設定/autonomy、task execution memory は session summaries/open items/rollback state を保持し、project/world memory はアクティブな project root と cached AgentDB analysis から explicit な prompt layer として注入されます。この project/world layer は recovery memory と adaptive routing にも使われ、semantic retrieval は引き続き live な関連 memory を別で追加します。
...
Replay correlation には chat-scoped な `taskRunId` も保存されるようになり、同じ chat 内で並行する task 同士が phase telemetry や recovery history を混ぜなくなりました。
