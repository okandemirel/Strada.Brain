...
**プロバイダーチェーン：** `PROVIDER_CHAIN` にプロバイダー名のカンマ区切りリストを設定します。Strada はコントロールプレーンのままで、この順序を主実行ワーカーとフォールバックの選択に使います。例：`PROVIDER_CHAIN=kimi,deepseek,claude` は Kimi を最初に使用し、Kimi が失敗すれば DeepSeek、次に Claude を使用します。
