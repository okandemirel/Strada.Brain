...
Strada は各タスクについて内部 execution journal と rollback memory も保持するようになりました。replan では最後の安定 checkpoint や行き詰まった branch を再利用でき、hardcoded な provider lore なしで adaptive phase scores を routing に戻します。
...
| `/routing info` | 現在のIDに対する最近のルーティング決定、runtime execution traces、phase outcomes、adaptive phase scores を表示（planning / execution / clarification-review / review / synthesis を含む） |
