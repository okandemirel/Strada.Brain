...
2. **メモリ検索** — 最も関連性の高い過去の会話3件を取得（TF-IDF）
3. **RAG検索** — C#コードベースのセマンティック検索（HNSWベクトル、上位6件）
4. **直感検索** — タスクに関連する学習済みパターンをプロアクティブに検索
5. **計画フェーズ** — LLMが学習済みインサイトと過去の失敗に基づいて番号付き計画を作成
6. **実行フェーズ** — LLMが計画に従ってツール呼び出しを実行
7. **観察** — 結果を記録、エラーリカバリが失敗を分析、障害分類器がエラーを分類
8. **振り返り** — 3ステップごと（またはエラー時）、LLMが判断：**続行**、**再計画**、または**完了**
9. **自動再計画** — 同種の連続3回以上の失敗で、失敗した戦略を避ける新しいアプローチを強制
10. **繰り返し** 完了まで最大50回の反復
11. **レスポンス送信** — チャネル経由でユーザーに送信（対応時はストリーミング）
...
**アクティブ検索（新機能）：** 各タスクの開始時に`InstinctRetriever`を使用して直感をプロアクティブに検索します。キーワード類似性とHNSWベクトル埋め込みで関連する学習済みパターンを検索し、計画フェーズのプロンプトに注入します。

**タスク分解：** 複雑なマルチステップリクエストはヒューリスティック分析により自動検出され、実行前にLLMを介して3-8の順序付きサブタスクに分解されます。
...
110 のテストファイルがカバー：エージェント、チャネル、セキュリティ、RAG、メモリ、学習、ダッシュボード、統合フロー。
...
src/
  ...
  agents/
    orchestrator.ts     # PAORエージェントループ、セッション管理、ストリーミング
  ...
  learning/             # パターンマッチング、信頼度スコアリング、インスティンクトライフサイクル、HNSWセマンティック検索
