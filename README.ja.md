...
最初の Web handoff が再起動と競合した場合でも、Strada はその起動を自動で再試行します。設定は保存済みなのにブラウザが timeout した場合は、もう一度保存するのではなく一度 refresh するか、`strada` を開き直して Web ダッシュボードを選んでください。
...
| ツール | 説明 |
|--------|------|
| `ask_user` | 複数選択肢と推奨回答付きの確認質問をユーザーに送信するが、実際に必要だと `clarification-review` が承認した場合にのみ使われる |
