...
Strada now also keeps an internal execution journal and rollback memory for each task. Replans can reuse the last stable checkpoint, remember exhausted branches, and feed adaptive phase scores back into provider routing without hardcoded provider lore.
...
| `/routing info` | Show recent routing decisions, runtime execution traces, phase outcomes, and adaptive phase scores for the current identity, including planning, execution, clarification-review, review, and synthesis phases |
