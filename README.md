...
If `./strada setup --web` detects an older Node runtime that cannot build the full portal bundle, Strada keeps web as the primary path: when `nvm` is available it can install a compatible Node.js version for you and continue directly into web setup after you approve it, temporarily filtering incompatible `prefix` / `globalconfig` npm settings during that guided upgrade; otherwise it opens the upgrade/download path and sends you back to web setup instead of silently dropping to terminal setup.
