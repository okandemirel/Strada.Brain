...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-3450%2B-brightgreen?style=flat-square" alt="테스트">
...
# 소스 checkout 전용 로컬 헬퍼 (global link 없이도 사용 가능)
npm run setup:web
...
모든 `npm` 명령은 `package.json` 이 있는 저장소 루트에서 실행해야 합니다. `ENOENT ... /Strada/package.json` 같은 오류가 보이면 한 단계 위 폴더에 있는 것이므로 먼저 `cd Strada.Brain` 하세요.
...
Strada.Brain은 매일 자동으로 업데이트를 확인하고 유휴 상태일 때 적용합니다. source checkout 과 `npm link` 설치는 git 로 업데이트됩니다. npm 기반 업데이트 명령은 public npm 배포가 있을 때만 적용됩니다.
