...
1. **메시지 도착** — 채팅 채널에서
2. **메모리 검색** — 가장 관련성 높은 과거 대화 3개 검색 (TF-IDF)
3. **RAG 검색** — C# 코드베이스 시맨틱 검색 (HNSW 벡터, 상위 6개)
4. **본능 검색** — 작업과 관련된 학습된 패턴을 사전 검색 (시맨틱 + 키워드 매칭)
5. **계획 단계** — LLM이 학습된 인사이트와 과거 실패를 반영하여 번호가 매겨진 계획 생성
6. **행동 단계** — LLM이 계획에 따라 도구 호출 실행
7. **관찰** — 결과 기록; 오류 복구가 실패 분석; 실패 분류기가 오류 분류
8. **반성** — 3단계마다 (또는 오류 시), LLM이 결정: **계속**, **재계획**, 또는 **완료**
9. **자동 재계획** — 동일 유형 실패가 3회 이상 연속되면, 실패한 전략을 피하는 새로운 접근 강제
10. **반복** 완료까지 최대 50회 반복
11. **응답 전송** — 채널을 통해 사용자에게 전송 (지원 시 스트리밍)
...
**능동적 검색 (신규):** 각 작업 시작 시 `InstinctRetriever`를 사용하여 본능을 사전 검색합니다. 키워드 유사성과 HNSW 벡터 임베딩으로 관련 학습 패턴을 검색하고 계획 단계 프롬프트에 주입합니다.

**작업 분해:** 복잡한 다단계 요청은 휴리스틱 분석으로 자동 감지되며, 실행 전 LLM을 통해 3-8개의 순서가 지정된 하위 작업으로 분해됩니다.
...
110개의 테스트 파일이 커버: 에이전트, 채널, 보안, RAG, 메모리, 학습, 대시보드, 통합 플로우.
...
src/
  ...
  learning/             # 패턴 매칭 (HNSW 시맨틱 검색), 신뢰도 스코어링, 인스팅트 라이프사이클
