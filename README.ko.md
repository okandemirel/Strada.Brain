...
Strada.Brain은 매일 자동으로 업데이트를 확인하고 유휴 상태일 때 적용합니다. 설치 방법 (npm 글로벌, npm 로컬, git 클론)을 감지하고 적절한 업데이트 전략을 사용합니다.
...
Strada.Brain은 [Strada.MCP](https://github.com/okandemirel/Strada.MCP) (76개 도구 Unity MCP 서버)를 감지하고 런타임 제어, 파일 작업, git, .NET 빌드, 코드 분석, 씬/프리팹 관리를 포함한 사용 가능한 MCP 기능을 에이전트에게 알립니다.
