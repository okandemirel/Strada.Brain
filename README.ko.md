...
| `/routing info` | 현재 ID에 대한 최근 라우팅 결정, runtime execution traces, phase outcomes, adaptive phase scores 표시 (verifier clean rate, rollback pressure, retry count, token-cost telemetry, provider catalog freshness, official alignment / capability drift 와 함께 planning, execution, clarification-review, review, synthesis 포함) |
