...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-3100%2B-brightgreen?style=flat-square" alt="테스트">
...
**베이지안 신뢰도 점수:**
- 본능은 **Beta 사후 추론** (`confidence = alpha / (alpha + beta)`)을 `Beta(1,1)` 무정보 사전 분포와 함께 사용
- 판정 점수 (0.0-1.0)가 정밀한 업데이트를 위한 분수 증거 가중치로 작용
- 블렌딩이나 시간 할인 없음 -- 순수 베이지안 사후 평균
...
| 도구 | 설명 |
|------|------|
| `strata_analyze_project` | C# 프로젝트 전체 스캔 -- 모듈, 시스템, 컴포넌트, 서비스 |
...
src/
  ...
  learning/
    ...
    scoring/
      confidence-scorer.ts  # 베이지안 Beta 사후 신뢰도, Elo, Wilson 구간
    matching/
