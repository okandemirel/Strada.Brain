...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4527%2B-brightgreen?style=flat-square" alt="테스트">
...
SQLite + HNSW 벡터 기반의 영구 메모리를 갖추고 있으며, 하이브리드 가중 신뢰도 점수를 활용하여 과거 에러로부터 학습하고, 복잡한 목표를 병렬 DAG 실행으로 분해하고, saga 롤백이 포함된 다중 도구 체인을 자동 합성하며, 사전 트리거가 포함된 24/7 데몬으로 운영할 수 있습니다. 채널/세션별 격리가 포함된 멀티 에이전트 오케스트레이션, 계층적 작업 위임, 자동 메모리 통합, 휴먼 인 더 루프 승인 게이트와 서킷 브레이커 보호 기능이 포함된 배포 하위 시스템, 그리고 Magic UI 컴포넌트(shadcn/ui + 21st.dev)로 구동되는 애니메이션 메트릭, 블러 전환, 영구 알림 센터를 갖춘 모던 글래스모피즘 웹 포털을 지원합니다.
...
- **Node.js 20.19+** (또는 **22.12+**) — Node.js가 설치되어 있지 않은 경우, 런처가 포터블 복사본의 자동 다운로드를 제안합니다 (Windows 전용, 약 30 MB 일회성 다운로드, `%LOCALAPPDATA%\Strada\node`에 저장). `STRADA_NODE_PATH`로 사용자 지정 바이너리를 지정할 수도 있습니다.
- 최소 하나의 지원되는 AI 공급자 자격 증명(`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` 등), Claude subscription 토큰(`ANTHROPIC_AUTH_MODE=claude-subscription` + `ANTHROPIC_AUTH_TOKEN`), OpenAI ChatGPT/Codex subscription 세션(`OPENAI_AUTH_MODE=chatgpt-subscription`), 또는 `ollama` 전용 `PROVIDER_CHAIN`
...
> **Node.js가 없어도 괜찮습니다.** Windows에서 시스템 PATH에 `node`가 없으면 런처(`strada.cmd` / `strada.ps1`)가 포터블 Node.js의 자동 다운로드를 제안합니다. 포터블 런타임은 `%LOCALAPPDATA%\Strada\node`에 저장되며 Strada 전용으로 사용됩니다. 시스템 PATH를 수정하거나 다른 도구에 영향을 주지 않습니다. 프롬프트를 건너뛰고 <https://nodejs.org>에서 직접 Node.js를 설치하거나, `set STRADA_NODE_PATH=C:\path\to\node.exe`로 기존 바이너리를 지정할 수도 있습니다.
...
Strada가 패키지된 npm/tarball 릴리스로 설치되면 런타임 설정은 현재 작업 디렉터리가 아니라 기본적으로 `~/.strada`(macOS/Linux) 또는 `%LOCALAPPDATA%\Strada`(Windows) 에 저장됩니다. 다른 app home 이 필요하면 `STRADA_HOME=/custom/path` 로 덮어쓸 수 있습니다.
...
Windows 에서는 같은 흐름이 `nvm-windows` 를 먼저, 그 다음 `winget` 을, 마지막으로 직접 Node 다운로드를 사용합니다. 이때 표시되는 재실행 명령은 `.\strada.ps1 setup --web` 입니다.
...
첫 웹 handoff 가 재시작 타이밍과 겹치면 Strada 는 그 시작을 자동으로 다시 시도합니다. 설정을 저장한 뒤에는 메인 앱이 준비될 때까지 같은 URL 에서 handoff 화면을 유지하므로 setup 을 다시 실행하지 마세요.

마법사는 Unity 프로젝트 경로, AI 공급자 접근 정보, 기본 채널, 언어를 묻습니다. `./strada setup` 은 이제 기본적으로 **Web Browser** 를 우선하며, 더 빠른 텍스트 흐름을 명시적으로 원할 때만 **Terminal** 을 선택하면 됩니다.
...
OpenAI 를 `chatgpt-subscription` 으로 사용할 때 setup 은 저장 전에 로컬 Codex/ChatGPT 세션을 검증합니다. 만료된 session 은 setup 단계에서 거부되고 `strada doctor` 에서도 보고됩니다.
Claude 를 `claude-subscription` 으로 사용할 때 setup 은 `claude auth login --claudeai` 와 `claude setup-token` 뒤에 생성한 `ANTHROPIC_AUTH_TOKEN` 을 요구하고, Anthropic 이 이 흐름을 Claude Code / Claude.ai 용으로 제한해 문서화한다는 경고를 표시하면서도 저장 전까지 선택한 worker 가 preflight 를 통과하도록 계속 요구합니다.
...
구성된 `UNITY_PROJECT_PATH` 는 Strada 가 코딩 작업을 수행할 때의 권위 있는 프로젝트 범위입니다. 에디터에 다른 Unity 프로젝트가 열려 있어도 Strada 는 시작 시 mismatch 경고만 표시할 수 있을 뿐, setup 에서 선택한 프로젝트 밖으로 조용히 전환하지 않습니다.
새 setup 은 이제 기본적으로 `MULTI_AGENT_ENABLED=true` 와 `TASK_DELEGATION_ENABLED=true` 를 함께 기록합니다. 레거시 단일 에이전트 경로가 필요하면 `MULTI_AGENT_ENABLED=false` 로 설정하세요. `TASK_DELEGATION_ENABLED=true` 여도 multi-agent 가 비활성화면 위임은 초기화되지 않습니다.
...
> **Windows 웹 설정 수정:** 이전 버전에서는 경로 구분자 버그로 인해 Windows에서 웹 설정 페이지가 빈 화면으로 표시되었습니다 (모든 정적 에셋이 경로 탐색 가드에 의해 차단됨). 이 문제는 수정되었습니다 — `.\strada.ps1 setup --web` 및 설정 후 `127.0.0.1:3000`의 웹 포털이 Windows에서 정상적으로 작동합니다. 이전에 Windows에서 터미널 설정으로 전환해야 했다면 이제 웹 설정이 권장됩니다.
...
git/source 설치에서는 source launcher 가 이미 동작하면 `strada doctor` 가 비어 있는 `dist/` 를 더 이상 blocking error 로 보지 않습니다. warning 으로만 보여 주고, 패키지된 build artifact 가 필요할 때만 repo root 기준 `npm run bootstrap` 명령을 안내합니다.

또는 `.env`를 수동으로 생성하세요:

```env
# Claude 를 API 키로 사용할 때
ANTHROPIC_API_KEY=sk-ant-...

# 또는 Claude 를 subscription 토큰으로 사용할 때
...
.\strada.ps1             # Windows PowerShell 용 source launcher
strada.cmd               # Windows 명령 프롬프트용 companion launcher
./strada install-command  # bare `strada` 명령을 사용자용으로 설치
./strada uninstall        # 설치된 bare 명령과 관리되는 PATH/profile 변경 제거
.\strada.ps1 uninstall   # Windows checkout 에서 같은 작업 수행
strada uninstall --purge-config # Strada 가 만든 repo-local runtime state 도 제거
...
.\strada.ps1 setup --web # PowerShell 에서 웹 마법사를 직접 시작
.\strada.ps1 setup --terminal # PowerShell 에서 터미널 마법사를 직접 시작
.\strada.ps1 doctor      # PowerShell 에서 doctor 실행
...
Strada.Brain은 매일 자동으로 업데이트를 확인하고 유휴 상태일 때 적용합니다. source checkout 과 `./strada install-command` 설치는 git 로 업데이트됩니다. git 업데이트가 성공하면 Strada 는 설치된 bare command wrapper 도 다시 써서 `strada` 가 현재 checkout 을 계속 가리키도록 유지합니다. npm 기반 업데이트 명령은 public npm 배포가 있을 때만 적용됩니다.
...
내장 웹 포털(`http://localhost:3000`)은 4가지 컨텍스트 적응형 모드를 갖춘 완전한 AI 워크스페이스를 제공합니다:

| 모드 | 단축키 | 설명 |
|------|--------|------|
| **채팅** | `Alt+1` | 파일 첨부, 음성 입력, 마크다운 렌더링을 지원하는 대화형 인터페이스 |
| **모니터** | `Alt+2` | 목표 분해, 작업 상태, 리뷰 파이프라인, 개입 제어의 실시간 DAG 시각화 |
| **캔버스** | `Alt+3` | tldraw를 활용한 비주얼 워크스페이스 — 에이전트 출력에서 자동 생성되는 9개의 커스텀 셰이프(CodeBlock, UMLClass, APIEndpoint, DataFlow 등) |
| **코드** | `Alt+4` | Monaco 에디터(멀티탭, 구문 강조), 파일 트리 탐색기, 터미널 출력을 갖춘 IDE 스타일 뷰 |

**자동 전환:** 포털은 에이전트 활동에 따라 자동으로 모드를 전환합니다 — 목표 실행 시 모니터가 열리고, 파일 쓰기 시 코드가 열리고, 시각적 출력 시 캔버스가 열립니다. 사용자는 수동 모드 선택으로 재정의할 수 있으며, 채팅 메시지를 보내면 재정의가 초기화됩니다.

**키보드 단축키:** `Alt+1-4` 모드 전환, `Cmd/Ctrl+B` 사이드바 토글, `Cmd/Ctrl+\` 보조 패널 토글, `Cmd/Ctrl+?` 단축키 도움말.

**기술 스택:** React 19, Vite, Tailwind CSS v4, shadcn/ui, Zustand, TanStack Query, Radix UI, ReactFlow, tldraw, Monaco Editor.

### Shadcn/UI + 21st.dev를 활용한 모던 UI

웹 포털은 shadcn/ui와 [21st.dev](https://21st.dev) 및 [Magic UI](https://magicui.design)의 커뮤니티 컴포넌트로 구동되는 프리미엄 글래스모피즘 디자인을 특징으로 합니다:

- **글래스모피즘 테마**: 모든 패널에 걸친 `backdrop-blur`, 반투명 서피스, 글로우 효과, 마이크로 인터랙션
- **Magic UI 컴포넌트**: NumberTicker (애니메이션 메트릭), BlurFade (페이지 전환), BorderBeam (활성 카드), TypingAnimation ("생각 중..." 인디케이터), SparklesText (브랜드 스파클), ShimmerButton (전송 시머), CoolMode (색종이 이스터에그)
- **Sonner 알림**: 자동 닫기, 심각도 색상, 실행 취소 액션 + 영구 알림 센터 시트가 포함된 토스트 시스템
- **접을 수 있는 관리자 네비**: 경로 인식 자동 확장으로 사이드바에서 접근 가능한 11개의 관리자 페이지
- **공유 기본 컴포넌트**: PageSkeleton, PageError, CopyButton, Sparkline, StatusDot, Badge, Sheet, ScrollArea, Input, Table
...
| 스킬 | 설명 | 요구사항 |
|------|------|---------|
| `notion` | 페이지 검색, 콘텐츠 읽기, 페이지 생성 | `NOTION_API_KEY` |
| `google-calendar` | 이벤트 나열, 생성, 오늘 일정 | `GOOGLE_CALENDAR_API_KEY` |
| `spotify` | 현재 재생, 검색, 재생 제어 | `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET` |
| `home-assistant` | 엔티티 상태, 토글, 서비스 호출 | `HA_URL`, `HA_TOKEN` |
...
- 헬스 체크 및 정상 종료 지원
- 비활성 시 legacy 단일 에이전트 경로와 동일하게 작동
...
새 setup 은 `TASK_DELEGATION_ENABLED=true` 를 기본으로 기록하지만, 위임은 `MULTI_AGENT_ENABLED=true` 일 때만 초기화됩니다.
...
- **관찰**: 등록된 옵저버 집합에서 환경 상태를 수집합니다. 현재 기본 daemon 배선은 trigger, user-activity, git-state 옵저버를 사용하며 build/test 옵저버는 해당 runtime 신호가 있을 때만 연결됩니다
...
권한 경계: Agent Core 는 프로액티브한 목표 생성기이자 알림 계층이지, PAOR 실행기의 병렬 대체물이 아닙니다. interactive/background 실행은 계속 orchestrator 의 PAOR 루프, verifier 파이프라인, 공유 loop-recovery 제어를 통과합니다.
...
Strada.Brain은 [Strada.MCP](https://github.com/okandemirel/Strada.MCP) (Unity MCP 서버)를 감지하고 런타임 제어, 파일 작업, git, .NET 빌드, 코드 분석, 씬/프리팹 관리를 포함한 사용 가능한 MCP 기능을 에이전트에게 알립니다. 현재 Brain runtime 에서 실제로 실행 가능한 MCP action tool만 worker tool surface에 들어가며, bridge/runtime 제약으로 막힌 MCP 기능은 계속 authoritative docs/resources로 취급됩니다.
...
지원되는 호스티드 공급자라면 사용할 수 있습니다. 아래 공급자는 모두 구현되어 있으며, 대부분은 API 키로 활성화됩니다. 대화용으로는 Claude subscription 토큰과 이 머신의 로컬 ChatGPT/Codex 구독 세션도 사용할 수 있습니다.

| 변수 | 공급자 | 기본 모델 |
|------|--------|-----------|
| `ANTHROPIC_API_KEY` | Claude API 키 | `claude-sonnet-4-20250514` |
| `ANTHROPIC_AUTH_MODE` | Claude 인증 모드 | `api-key`(기본값) 또는 `claude-subscription` |
| `ANTHROPIC_AUTH_TOKEN` | Claude subscription bearer 토큰 | `ANTHROPIC_AUTH_MODE=claude-subscription`일 때 `claude setup-token`으로 생성 |
...
`claude`는 `ANTHROPIC_API_KEY` 또는 `ANTHROPIC_AUTH_MODE=claude-subscription`과 `ANTHROPIC_AUTH_TOKEN` 조합으로 동작할 수 있고, `openai`는 `OPENAI_API_KEY` 또는 로컬 ChatGPT/Codex subscription 세션으로 동작할 수 있습니다.
...
Strada 는 이제 각 작업마다 내부 execution journal 과 rollback memory 도 유지합니다. replan 은 마지막 안정 checkpoint 와 소진된 branch 에 더해 project/world anchor 도 다시 참고할 수 있고, hardcoded provider lore 없이 adaptive phase scores 를 routing 에 되돌려 줍니다. 이 score 는 verifier clean rate, rollback pressure, retry count, repeated failure fingerprints, repeated world-context failures, phase-local token cost, provider catalog freshness, shared catalog 기준 official alignment / capability drift 도 함께 반영합니다.
메모리도 이제 역할별로 분리됩니다. user profile state 는 이름/선호/autonomy 를, task execution memory 는 session summaries/open items/rollback state 를 담당하고, project/world memory 는 활성 project root 와 cached AgentDB analysis 에서 explicit prompt layer 로 주입됩니다. task execution memory 는 활성 identity 의 `latest snapshot` 만 유지하고, 정확한 task run 의 `persisted chronology` 는 여기 두지 않습니다. 이 project/world layer 는 recovery memory 와 adaptive routing 도 함께 지원하고, semantic retrieval 은 live 관련 memory 를 계속 별도로 더합니다.
...
Replay correlation 도 이제 chat-scoped `taskRunId` 와 함께 persist 되므로, 같은 chat 안에서 동시에 도는 task 들이 phase telemetry 나 recovery history 를 서로 섞지 않습니다. 정확한 task run 의 `persisted chronology` 는 같은 `taskRunId` 로 찾는 learning trajectories / replay contexts 쪽에 남습니다.
같은 learning path 는 이제 runtime self-improvement artifacts 도 materialize 합니다. 반복되는 high-confidence pattern 은 먼저 `skill`, `workflow`, `knowledge_patch` 형태의 `shadow` state 로 기록되고, verifier-backed clean shadow evaluations 를 통과한 경우에만 `active` guidance 로 승격됩니다. `/routing info` 와 dashboard 는 현재 프로젝트 기준 identity-scoped telemetry 로 state, sample 수, clean/retry/failure 분포를 보여 줍니다.
...
`ANTHROPIC_AUTH_MODE=claude-subscription`은 로컬 Claude 로그인(`claude auth login --claudeai` 후 `claude setup-token`)에서 생성한 bearer 토큰을 사용합니다. Anthropic 은 claude.ai subscription auth 를 Claude Code 와 Claude.ai 로 제한된 흐름으로 문서화하므로, 이 모드는 사용자 책임의 고급 옵션으로 제공됩니다.
...
| `memory_search` | 영구적인 visible 대화 메모리 검색 |
...
| 도구 | 설명 |
|------|------|
| `ask_user` | 일반 worker action tool이 아니며, `clarification-review` 가 정말 필요하다고 승인한 경우에만 user-facing 으로 surface 된다 |
| `show_plan` | 일반 worker action tool이 아니며, 사용자가 먼저 계획 검토를 명시적으로 요청한 경우에만 plan-review turn 으로 surface 된다 |
...
| `/routing info` | 최근 routing decisions, runtime execution traces, phase outcomes, adaptive phase scores 와 함께 현재 프로젝트 기준 identity-scoped runtime self-improvement telemetry 를 표시한다 (verifier clean rate, rollback pressure, retry count, token-cost telemetry, provider catalog freshness, official alignment / capability drift, artifact promotion telemetry 포함) |
