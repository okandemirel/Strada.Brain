...
### Removed
- **Agent Core v1 engine DELETED (cutover Step 5)**: `runAgentLoop`, `runBackgroundTask`, `runWorkerTask`, their loop-only helpers, `V1AgentRunner`, and the v1 rollout flag sets are gone (~3,900 lines); the V2 spine is THE engine on every route with no fallback. `selectAgentRunner` now always constructs `V2AgentRunner` and throws a descriptive error for hosts without the Agent Core wiring hooks. Ops safety: a stale `AGENT_CORE_FLAG_SET` revert value (`all-v1`, `v1-driver+full-control-plane`, the rollout-era partial sets) resolves to the production default instead of crash-looping at boot. Behavior notes: `STREAMING_ENABLED` no longer gates the engine (v2 streams whenever the provider supports it — the resilience-correct default), and goal-tree persistence moved into the shared decomposition helper so every decomposition path persists

### Changed
- **Agent Core v2 — THE FLIP (production default now runs the V2 engine)**: with `AGENT_CORE_FLAG_SET` unset, the default flag set is now `v2-all-routes+full-control-plane` — the unified `V2AgentRunner` spine drives every route (interactive, background, worker, supervisor-node) on the full control plane (`failureLedger`, `runClock`, `silenceAccumulator`, `typedCancelReason`). Supersedes the "unset → `all-v1` → byte-identical to v1" claim in the Added entry below — that described the pre-flip rollout scaffolding
...
### Added
- **Agent Core v2 — control-plane migration (Phases 0–1, behind default-off flags)**: a strangler-fig rewrite of the reactive agent loop's three drift-prone seams into one coherent fabric in `src/agent-core/control/` + `src/agent-core/runner/`, **without** discarding v1's incident-hardening (3h27m runaway, ~70min stall, audits #6–#18). Landed so far: the `AgentRunner` seam + `V1AgentRunner` pass-through (Phase 0, rerouting worker/background/supervisor-node through one façade, `__workerCollector` superseded by a structured `AgentRunResult`), and Phase 1's control plane wired into the live v1 loop one concern at a time — `FailureLedger` (a single deterministic verdict arbiter wrapping the kept `IterationHealthTracker`), `RunClock` (per-call deadlines carved subtractively from task-remaining + a livelock-proof task-scope *silence accumulator*, replacing the scattered per-call `AbortSignal.timeout` sites), the silence-accumulator task-inactivity verdict (bounds the delegation livelock v1 never caught), and the typed `CancelReason` union. Each concern is gated by an enumerated, **reject-at-boot** `LEGAL_FLAG_SET` and **defaults OFF**, so shipped behavior is byte-identical to v1; the v2 paths are unit- and incident-regression-verified (the runaway + stall reproduced under a deterministic `FakeClock`) before any flag is flipped. **Phase 2 has since shipped** the unified `V2AgentRunner` spine + `ModelGateway` + run-scoped `EventBus` + a faithful `OrchestratorPort` (compose+adapt over the existing orchestrator helpers, not extraction), making the **worker + supervisor-node** routes flip-ready: the spine runs them via the real port with the workspace lease threaded for git-worktree isolation, the reflection-decision / assistant-text / tool-exec-mode divergences fixed, covered by a real-port characterization matrix (REFLECTING boundary, max-tokens runaway guard, lease isolation). Flipping a route is an **`AGENT_CORE_FLAG_SET`** config change (e.g. `v2-worker-only+full-control-plane`); unset → `all-v1` → byte-identical to v1, and a flip starts the soak that gates v1 deletion. The interactive route (`handleMessage`), Phase 3 (scoring/capability unification), and Phase 5 (streaming visibility) remain pending. See `plans/agent-core-v2/` and `src/agent-core/README.md`
